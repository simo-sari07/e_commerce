...
import { ShoppingCart, Star, ChevronLeft, ChevronRight, ArrowLeft, Eye, Heart } from "lucide-react"
    ...
    <div className="bg-white rounded-xl shadow-md overflow-hidden group relative transition-all duration-300 hover:shadow-xl">
        ...
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <div className="absolute bottom-4 left-0 right-0 flex justify-center space-x-3 transform translate-y-full group-hover:translate-y-0 transition-transform duration-500">
            ...
            <button
              ...
              className={`p-3 bg-white rounded-full hover:bg-[#9e121b] hover:text-white transform hover:-translate-y-1 transition-all duration-300 shadow-lg hover:shadow-xl ${isInWishlist ? "text-[#b91c1c]" : ""}`}
      ...
      <div className="p-4">
        ...
        <div className="flex items-baseline gap-2 mt-4">
          <p className="text-xl font-bold text-gray-900">{product.new_price} Dhss</p>
          {product.old_price > 0 ? <p className="text-sm text-gray-500 line-through">{product.old_price} Dhse</p> : ""}
        </div>
      </div>
    </div>
  )
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        const relatedResponse = await fetch(`http://localhost:5000/api/products?category=${data.category}&limit=5`)
          ...
          <div className="grid md:grid-cols-2 gap-8 p-8">
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-center">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`h-5 w-5 ${
                        i < Math.floor(product.rating || 5) ? "text-yellow-400 fill-yellow-400" : "text-gray-200"
                      }`}
                    />
                  ))}
                  <span className="ml-2 text-sm text-gray-600">({product.reviews || 140})</span>
              ...
              <div className="flex items-baseline gap-4">
                <span className="text-3xl font-bold text-gray-900">{product.new_price} Dhs</span>
                {product.old_price > 0 ? (
                  <span className="text-xl text-gray-500 line-through">{product.old_price} Dhs</span>
                ) : (
                  ""
                )}{" "}
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {relatedProducts.map((relatedProduct) => (
                <ProductCard
                  ...
                  isInWishlist={wishlistItems.some((item) => item._id === relatedProduct._id)}
                />
              ))}
            </div>
          </div>
